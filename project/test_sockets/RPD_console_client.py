...
def DisplayConsole(stdscr, args):
    ...
    csv_file = csv.writer(csv_file_header, dialect='excel', delimiter=',', quotechar="'", quoting=csv.QUOTE_MINIMAL,
                          lineterminator='\n')
    ...
    # calculate the positions of all data display cells
    # 3 per rows - 33 chars each cell
    row_display = 25
    data_row_offset = 3
    col_display = 2
    data_col_offset = 2
    col_width = int((width - 6 * data_col_offset) / col_display)
    data_coord = []

    for rw in range(row_display):       # rows
        for cl in range(col_display):   # columns
            label_row_pos = rw + data_row_offset
            data_row_pos = rw + data_row_offset
            label_col_pos = data_col_offset + cl * col_width
            data_col_pos = data_col_offset + cl * col_width + int(col_width / 2)
            data_coord.append((label_row_pos,label_col_pos,data_row_pos,data_col_pos))
    ...
    for col, channel in channels_dict.items():
        ...
        stdscr.addstr(data_coord[int(col)][0], data_coord[int(col)][1], col_head[:int(col_width/2)])
        stdscr.attroff(curses.color_pair(3))
    # adding the timestamp label not present in configuration
    stdscr.attron(curses.color_pair(3))
    stdscr.addstr(data_coord[0][0], data_coord[0][1], "UNIX Timestamp")
    stdscr.attroff(curses.color_pair(3))
    stdscr.refresh()
    ...
    while gpio.input(gpio_key_sense) == KEY_ON:
        try:
            ...
            if first_cycle:
                ...
                # Render status bar
                statusbarstr = "STATUS BAR"[:width-1]
                ...
                answ_PL1012 = client_PL1012.recv(24)
                ...
                pl1012_data = PL1012DataFormat.parse(answ_PL1012)
                ...
                for col, channel in channels_dict.items():
                    if channel['hw'] == 'tc08':
                        ...
                        data_row[int(col)] = linearise_signal(pl1012_data.data[int(channel['ph'])-1], channel)
                        ...
                        for msg in CAN_row_Data:
                            if len(msg) > 1:
                                ...
                                if channel['pgn'] in msg_id.upper():
                                    ...
                                    data_row[int(col)] = linearise_signal(
                                        msg[int(channel['byte']):int(channel['byte']) + data_len], channel)
                    else:
                ...
                statusbarstr = "Samples {:0>9} | Filename: {: <30}".format(cycle, csv_file_name.split('/')[-1])[:width - 1]
...
def linearise_signal(raw_value,parameters):
    ...
    if parameters['hw'] == "pl1012":
        # linearized_value = raw_value
        linearized_value = raw_value*2.5/4092*float(parameters['slope'])+float(parameters['offset'])
...
if __name__ == '__main__':
    ...
    gpio_pwrltch_out = 20
    gpio.setup(gpio_pwrltch_out, gpio.OUT,
               initial=gpio.HIGH)  # this the power latch enable output - initialised to HIGH
    ...
    curses.wrapper(DisplayConsole, mode  )
